...
const STAGES: { id: LifecycleStage; label: string; short: string; coverage: string }[] = [
  { id: "F1", label: "F-1 / OPT", short: "Study & work", coverage: "Beta" },
  ...
  { id: "LPR", label: "Permanent resident", short: "Maintain & renew", coverage: "Beta" },
  { id: "CITIZENSHIP", label: "Citizenship", short: "Naturalization", coverage: "Beta" },
];
    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        <header className="topbar">
          <Brand mobile />
          <div className="topbar-copy"><span className="coverage-dot" /><span>{plan?.coverage ?? stage.coverage} module</span><b>Dates from your profile · Review official guidance</b></div>
...
function PlanView({ profile, plan, documents, cases, onEdit, onNavigate, onRefresh, notify }: { profile: ImmigrationProfile; plan: PlanResult | null; documents: SavedDocument[]; cases: TrackedCase[]; onEdit: () => void; onNavigate: (view: View) => void; onRefresh: () => Promise<void>; notify: (message: string) => void }) {
  ...
  const exportCalendar = () => {
    ...
    link.href = url; link.download = "immigrant-os-dates.ics"; link.click();
    window.setTimeout(() => URL.revokeObjectURL(url), 1000);
    notify("Calendar downloaded. Import it into your calendar; future changes are not synced automatically.");
  };
  ...
  return <div className="page plan-page">
    <section className="page-intro"><div><p className="eyebrow">{new Intl.DateTimeFormat("en-US", { weekday: "long", month: "long", day: "numeric" }).format(new Date()).toUpperCase()}</p><h1>{greeting()}, {firstName(profile.displayName)}.</h1><p>{plan?.summary}</p></div><div className="intro-actions"><button className="secondary" onClick={onEdit}><Icon name="edit" /> Update profile</button><button className="secondary" onClick={exportCalendar} disabled={!openItems.some((item) => item.dueDate)}><Icon name="download" /> Export calendar</button><button className="icon-action" onClick={() => window.print()} aria-label="Print action plan"><Icon name="print" /></button></div></section>

    {plan?.missingFields.length ? <section className="missing-card"><div className="missing-icon"><Icon name="alert" /></div><div><span className="overline">COMPLETE YOUR TIMELINE</span><h2>{plan.missingFields.length} profile item{plan.missingFields.length === 1 ? "" : "s"} missing</h2><ul>{plan.missingFields.map((field) => <li key={field}>{field}</li>)}</ul><button onClick={onEdit}>Add missing details <Icon name="arrow" /></button></div></section> : null}

    {next ? <section className={`next-action ${next.severity}`}><div className="next-date"><span>{next.dueDate ? month(next.dueDate) : "NEXT"}</span><strong>{next.dueDate ? day(next.dueDate) : "→"}</strong><small>{next.dueDate ? year(next.dueDate) : "ACTION"}</small></div><div className="next-copy"><div className="tag-row"><span className={`tag ${next.severity}`}>{next.dueDate && next.dueDate < localToday() ? "Past date · review" : next.severity}</span><span className={`confidence ${next.confidence}`}>{labelConfidence(next.confidence)}</span></div><h2>{next.title}</h2><p>{next.description}</p><div className="basis"><span>Calculated from</span>{next.basis.map((value) => <b key={value}>{value}</b>)}</div><a href={next.source.url} target="_blank" rel="noreferrer">{next.source.label} <Icon name="external" /></a></div><button className="complete-button" disabled={busy === next.id} onClick={() => toggle(next)}><Icon name="check" /> Mark complete</button></section> : <section className="all-done"><Icon name="check" /><h2>You’re caught up.</h2><p>There are no open actions in the supported rules for this profile.</p></section>}
...
function CasesView({ cases, onChanged, notify }: { cases: TrackedCase[]; onChanged: () => Promise<void>; notify: (message: string) => void }) {
  ...
  return <div className="page"><section className="page-intro"><div><p className="eyebrow">CASE WORKSPACE</p><h1>USCIS receipts</h1><p>Keep each filing separate and open the official status tool with the correct receipt number.</p></div></section><form className="case-form" onSubmit={submit}><div><label>Receipt number</label><input value={receiptNumber} onChange={(event) => setReceiptNumber(event.target.value.toUpperCase())} placeholder="IOE1234567890" maxLength={15} required /><small>3 letters + 10 digits</small></div><div><label>Form</label><input value={formType} onChange={(event) => setFormType(event.target.value)} placeholder="I-765" /></div><div><label>Label</label><input value={nickname} onChange={(event) => setNickname(event.target.value)} placeholder="My OPT EAD" /></div><button className="primary" disabled={saving}><Icon name="plus" /> {saving ? "Saving…" : "Add case"}</button></form>
    ...
    <section className="info-strip"><Icon name="info" /><div><strong>What works now</strong><p>Save your receipts here and open USCIS to check each case. Status updates are not fetched automatically.</p></div></section>
  </div>;
}
...
function AssistantView({ plan }: { plan: PlanResult | null }) {
  ...
  const suggestions = ["What should I do next?", "When does my status expire?", "What information is missing?", "What dates have I saved?"];
...
function Onboarding({ onSaved }: { onSaved: () => void }) {
  ...
  return <main className="onboarding-shell"><header><Brand /><span>Private beta · General information only</span></header><div className="onboarding-grid"><section className="onboarding-copy"><p className="eyebrow">YOUR DATES, IN ONE PLACE</p><h1>Know what matters next.</h1><p>Start with the dates you know. Save a personal checklist, track your receipts, and export dates to your calendar. Document upload is optional; you can add more details later.</p><div className="trust-list"><div><Icon name="plan" /><span><strong>Your saved dates</strong><small>Review the inputs and official guidance.</small></span></div><div><Icon name="shield" /><span><strong>A simple checklist</strong><small>Mark tasks done and pick up where you left off.</small></span></div><div><Icon name="spark" /><span><strong>Start small</strong><small>Choose your current stage; add details later.</small></span></div></div></section><form className="onboarding-card" onSubmit={save}><div className="step-label"><span>1</span><div><small>START HERE</small><strong>Where are you now?</strong></div></div><div className="stage-grid">{STAGES.map((stage) => <button type="button" className={draft.lifecycleStage === stage.id ? "selected" : ""} onClick={() => stage.id !== draft.lifecycleStage && setDraft({ ...draft, lifecycleStage: stage.id, statusSubtype: stageDefaults(stage.id, draft.displayName).statusSubtype })} key={stage.id}><span><b>{stage.label}</b><small>{stage.short}</small></span><em>{stage.coverage}</em></button>)}</div><div className="step-label second"><span>2</span><div><small>MINIMUM INPUTS</small><strong>Build your first timeline</strong></div></div><ProfileFields draft={draft} setDraft={setDraft} compact />{error && <p className="form-error">{error}</p>}<button className="primary wide" disabled={saving}>{saving ? "Building your plan…" : "Create my action plan"} <Icon name="arrow" /></button><p className="form-disclaimer">Your entries are saved to your signed-in account. Start without uploading documents. This checklist is not legal advice or an eligibility decision.</p></form></div></main>;
}
...
function ProfileFields({ draft, setDraft, compact = false }: { draft: ImmigrationProfile; setDraft: (profile: ImmigrationProfile) => void; compact?: boolean }) {
  ...
  return <div className={`profile-fields ${compact ? "compact" : ""}`}><label className="span-2"><span>Your name</span><input value={draft.displayName} onChange={(event) => field("displayName", event.target.value)} placeholder="Name or nickname (optional)" /></label>
    {draft.lifecycleStage === "F1" && <><label><span>Current F-1 stage</span><select value={draft.statusSubtype} onChange={(event) => field("statusSubtype", event.target.value)}><option value="STUDENT">Student</option><option value="OPT">Post-completion OPT</option><option value="STEM_OPT">STEM OPT</option></select></label><label><span>I-94 admission</span><select value={draft.admissionBasis} onChange={(event) => field("admissionBasis", event.target.value)}><option value="DS_TRANSITION">D/S (duration of status)</option><option value="FIXED_DATE">Fixed admit-until date</option></select></label><DateField label="I-20 program end" value={draft.programEndDate} onChange={(value) => field("programEndDate", value)} />{draft.admissionBasis === "FIXED_DATE" && <DateField label="I-94 admit-until" value={draft.i94ExpirationDate} onChange={(value) => field("i94ExpirationDate", value)} />}{draft.statusSubtype !== "STUDENT" && <DateField label="Current EAD expires" value={draft.eadEndDate} onChange={(value) => field("eadEndDate", value)} />}</>}
    {draft.lifecycleStage === "H1B" && <><label><span>Status</span><select value={draft.statusSubtype} onChange={(event) => field("statusSubtype", event.target.value)}><option value="H1B">H-1B</option><option value="H4">H-4</option></select></label><DateField label="I-94 admit-until" value={draft.i94ExpirationDate} onChange={(value) => field("i94ExpirationDate", value)} /><DateField label="Visa stamp expires" value={draft.visaExpirationDate} onChange={(value) => field("visaExpirationDate", value)} /></>}
...
function DateField({ label, value, onChange, required = false }: { label: string; value: string | null; onChange: (value: string | null) => void; required?: boolean }) {
  return <label><span>{label}</span><input type="date" value={value ?? ""} onInput={(event) => onChange(event.currentTarget.value || null)} required={required} /></label>;
}
...
function ErrorScreen({ message, onRetry }: { message: string; onRetry: () => void }) { return <main className="center-screen error-screen"><span className="error-mark"><Icon name="alert" /></span><h1>Workspace unavailable</h1><p>{message}</p><button className="primary" onClick={onRetry}>Try again</button><a href="/signin-with-chatgpt?return_to=%2F">Sign in with ChatGPT</a></main>; }
...
function labelConfidence(value: PlanItem["confidence"]) { return value === "needs-review" ? "Needs review" : value === "preliminary" ? "Preliminary" : "Review source"; }
